  ...
  return (
    <div className="bg-white border border-slate-200 shadow-sm rounded-xl flex flex-col overflow-hidden h-full">
      <div className="flex items-center gap-2 px-4 py-2 border-b border-slate-200 shrink-0 bg-slate-50/50">
        ...
        <Tooltip content="Live battle log showing Red Team attacks, Blue Team detections, and RL mutation feedback per transaction.">
          ...
          <tbody className="divide-y divide-slate-100">
            {instances.map((inst, i) => (
              <tr key={`${inst.txnId}-${i}`} className="hover:bg-slate-50/50 transition-colors">
                ...
                {/* Initial Risk */}
                <td className="px-2 py-1.5 font-mono text-aegis-red text-center font-semibold">
                ...
                <td className="px-2 py-1.5">
                  <div className="relative w-full h-5 flex items-center justify-center">
                    <div className="absolute left-0 right-0 top-1/2 -translate-y-1/2 h-[3px] rounded-full bg-gradient-to-r from-aegis-blue-light/50 to-aegis-red-light/50 z-0" />
                    <div className="absolute left-1/2 top-0 bottom-0 w-[2px] -translate-x-1/2 bg-slate-400 border-x border-white z-0" />
                    
                    <div 
                      className="absolute w-1.5 h-1.5 rounded-full bg-aegis-red border border-white z-10" 
                      style={{ left: `calc(${inst.originalRisk * 100}% - 3px)` }}
                    />
                    
                    <motion.div 
                      className={`absolute w-1.5 h-1.5 rounded-full border border-white z-20 ${inst.isEvaded ? 'bg-aegis-blue' : 'bg-aegis-red'}`}
                ...
                {/* Final Risk */}
                <td className={`px-2 py-1.5 font-mono text-center font-bold ${inst.isEvaded ? 'text-aegis-blue' : 'text-aegis-red'}`}>
                ...
                <td className="px-2 py-1.5">
                  <div className="flex justify-center">
                    {inst.attackVector === "Normal Traffic" ? (
                      ...
                      <span className="flex items-center gap-0.5 text-[8px] font-bold text-aegis-red bg-aegis-red-light/30 px-1.5 py-0.5 rounded border border-aegis-red/20">
                        <ShieldX className="w-2.5 h-2.5" /> EVADED
                      </span>
                    ) : (
                      <span className="flex items-center gap-0.5 text-[8px] font-bold text-aegis-blue bg-aegis-blue-light/30 px-1.5 py-0.5 rounded border border-aegis-blue/20">
