...
class CTGANMimicryAgent(BaseAgent):
    def generate_transactions(self, start: datetime, end: datetime, merchants: List[str], accounts: List[Account]) -> List[Transaction]:
        txns = []
        current = start
        while current < end:
            # Samples from a simulated CTGAN latent space to perfectly match legitimate covariant distributions.
            if random.random() < 0.1:
                ...
                txn = Transaction(
                    ...
                    is_fraud=True,
                    fraud_type="ctgan_mimicry",
                    fraud_campaign_id="ctgan_mimicry_campaign_1",
                ...
                txn.is_fraud = True # Labels it fraud but stats match benign exactly
                self.history.append(txn)
                txns.append(txn)
                current += timedelta(hours=random.randint(24, 48))
            else:
