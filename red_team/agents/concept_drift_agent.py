...
class ConceptDriftAgent(BaseAgent):
    def generate_transactions(self, start: datetime, end: datetime, merchants: List[str], accounts: List[Account]) -> List[Transaction]:
        ...
        current = start
        while current < end:
            # Shifts the mean time-of-day distribution slowly to induce Covariant Shift.
            if random.random() < 0.05:
                ...
                txn = Transaction(
                    ...
                    mcc_code="0000",
                    mcc_description="Drifting Fraud",
                    ...
                    is_fraud=True,
                    fraud_type="concept_drift",
                    fraud_campaign_id="concept_drift_campaign_1",
                ...
                txn.hour_of_day = (current.hour + int((current - start).days / 5)) % 24 # Drifts 1 hour every 5 days
                self.history.append(txn)
                txns.append(txn)
                current += timedelta(days=random.randint(1, 2))
            else:
