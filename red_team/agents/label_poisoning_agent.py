...
class LabelPoisoningAgent(BaseAgent):
    def generate_transactions(self, start: datetime, end: datetime, merchants: List[str], accounts: List[Account]) -> List[Transaction]:
        txns = []
        current = start
        while current < end:
            # Generates benign-looking transactions with fraudulent intent to slowly degrade Blue Team decision boundaries.
            if random.random() < 0.02:
                ...
                txn = Transaction(
                    ...
                    is_fraud=True,
                    fraud_type="label_poisoning",
                    fraud_campaign_id="label_poisoning_campaign_1",
                ...
                txn.is_fraud = random.random() < 0.5 # Flip labels 50% of time to confuse retrainer
